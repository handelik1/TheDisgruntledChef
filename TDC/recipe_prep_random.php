...
require('connect.php');
	...
	$ingredientQuery = mysqli_query($con, "select * from ingredients where recipe_ID = ". $random);
	$recipeQuery = mysqli_query($con, "select * from recipes where recipe_ID =" . $random);
	...
	$outstr .= '<input name="prep-ingredient'.$c.'" class = "prep-ingredient" id = "prep-ingredient'.$c.'" type = "checkbox" value = "'.$ingredient['general_name'].'"/><label id = "ingredient-label" for = "prep-ingredient'.$c.'"><img id = "ingredient-pic" src = "./img/'.$ingredient['image'].'"/>';
	$outstr .= '</label>';
			$c++;
			}
	$outstr .= '</div>';
	...
	$outstr .= '<form action = "recipe.php" method ="post">
			    <input type = "hidden" name = "recipe_ID" value = "'.$random.'">; 
